...
const Home2 = (props) => {
  ...
  const showToast = () => {
    Toast.show({
      type: 'success',
      text1: 'Enter reason for cancellation',
...
/> */}
        ...
        <View style={{
          ...
          <TouchableOpacity onPress={() => { props.navigation.navigate('Home3') }} style={{ top: -15, width: '100%', backgroundColor: '#fee1be', borderRadius: 10, flexDirection: "row", justifyContent: 'space-between' }}>
          ...
          <View style={{ height: 350, backgroundColor: '#fff', borderTopLeftRadius: 15, borderTopRightRadius: 15, position: 'absolute', bottom: 0, width: '100%', borderColor: '#fff', borderWidth: 0.3, alignSelf: 'center' }}>
            ...
            <View style={{ alignSelf: 'center', width: '90%', marginTop: 36 }}>
              <MyButtons title="Save" height={50} width={'100%'} borderRadius={5} press={() => {
                ...
                if (datevalue == '1') {
                  if (reason === '') {
                    Toast.show({ text1: 'Please enter the reason for cacellation' });
                    setmodlevisual(false)
                    return
                  }
                  ChangeRideStatus(datevalue)
