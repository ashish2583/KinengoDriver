...
const Otp = (props) => {
  ...
  const optclickedForgot = async () => {
    if (props.route.params.otp == value || resendOtps == value) {
      ...
      console.log("data", data);
      setLoading(true)
      const { responseJson, err } = await requestPostApi(Password_otp, data, 'POST', '')
      setLoading(false)
      console.log('the res==>>', responseJson)
      if (responseJson.headers.success == 1) {
        props.navigation.navigate('ResetPassword', { emailid: responseJson.body.emailid, from: 'Otp' })
